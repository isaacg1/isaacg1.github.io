...
\section{\centerline{PAPERS UNDER REVISION (OR SUBMISSION)}}
    ...
    Isaac Grosof and Hayriye Ayhan. ``Multiserver-job Response Time under Multilevel Scaling''.

    \textbf{Ziyuan Wang} and Isaac Grosof. ``Novel Lower Bounds on M/G/k Scheduling''.

    Céline Comte and Isaac Grosof. ``Graph-Based Product Form''. \\
    Note: Joint first authors.

    Runhan Xie, Ziv Scully, Isaac Grosof. ``Optimal Multiserver Scheduling under General Service Constraints in Heavy Traffic''.

    Yashaswini Murthy, Isaac Grosof, Siva Theja Maguluri, R. Srikant. ``Performance of NPG in Countable State-Space Average-Cost RL''.
...
\section{\centerline{OTHER EMPLOYMENT}}
    \vspace{4pt}
\textbf{October 2024}: Panel speaker on supporting trans researchers. \\
...
- Pyth is one of the best programming languages for code golfing, i.e. solving tasks with the shortest possible programs.\\
